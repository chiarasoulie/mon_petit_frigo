  ...
  fetch(url, fetchOptions)
    ...
    .then((dataJSON) => {
      ...
      dataJSON.forEach((aliment) =>
        listeAliments.push(
          new Aliment(aliment.id, aliment.nom, aliment.qte, aliment.photo),
        ),
      );
    })
    .catch((error) => {
  ...
  fetch(url + "?search=" + motcle, fetchOptions)
    ...
    .then((dataJSON) => {
      ...
      for (let a of alimentsTrouves) {
        /* pour chaque aliment, on récupère ses attributs et on l'insère dans l'html */
        document.getElementById("recherche").innerHTML +=
          "<li>" + a.nom + " quantité : " + a.qte + "</li>";
      }
...
<template>
  <body>
    <h2>Liste d'aliments dans le frigo</h2>
    <br />
    <Item
      v-for="aliment of listeAliments"
      :key="aliment.id"
      :aliment="aliment"
